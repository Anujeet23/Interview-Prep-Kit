...
export function geminiProvider({ apiKey, model }) {
  return {
    ...
    async complete({ system, messages, maxTokens = 2048, temperature = 0.3, json = true }) {
      ...
      if (!text) throw new ProviderError('BAD_RESPONSE', `Empty response (finishReason: ${cand?.finishReason || data.promptFeedback?.blockReason || 'unknown'})`);
      return { text, truncated: cand?.finishReason === 'MAX_TOKENS', usage: data.usageMetadata };
    },
  };
}
...
function classify(res, data) {
  ...
  if (res.status === 429) {
    ...
    return new ProviderError(perDay ? 'QUOTA_EXHAUSTED' : 'RATE_LIMITED', msg, { retryAfterMs: parseDelay(retry), status: 429 });
  }
  if (res.status === 401 || res.status === 403) return new ProviderError('AUTH', msg, { status: res.status });
  if (res.status >= 500) return new ProviderError('SERVER', msg, { status: res.status });
