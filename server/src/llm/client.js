...
export function createLLM({
  ...
  async function viaSlot(
    slot,
    { task, system, user, schema, data, maxTokens, temperature },
  ) {
    ...
    for (;;) {
      const est =
        estimateTokens(system + messages.map((m) => m.content).join("\n")) +
        maxTokens;
      ...
      let out;
      try {
        out = await slot.provider.complete({
          system,
          messages,
          maxTokens,
          temperature,
        ...
        const wait =
          e.retryAfterMs ??
          Math.min(30_000, 2000 * 2 ** (transient - 1)) +
            Math.floor(Math.random() * 500);
        if (e.code === "RATE_LIMITED") {
          stats.rate_limited++;
          slot.limiter.penalise(wait);
        }
        if (transient > 6 || Date.now() - started + wait > maxWaitMsPerCall)
          throw e;
        stats.retries++;
        onEvent({
          type: "retry",
      ...
      try {
        ...
        if (repairs >= 2)
          throw new ProviderError(
            "INVALID_OUTPUT",
            `Model output failed validation ${repairs + 1} times: ${describeZodError(err) || err.message}`,
          );
        repairs++;
        stats.repairs++;
  ...
  async function generateJSON({
    ...
    for (const [i, slot] of usable.entries()) {
      try {
        const result = await viaSlot(slot, {
          task,
          system,
          user,
          schema,
        ...
        errors.push(
          `${slot.provider.name}: ${e.code || ""} ${e.message}`.trim(),
        );
        if (e.code === "QUOTA_EXHAUSTED")
          slot.disabledUntil = Date.now() + 15 * 60_000;
    ...
    throw new AppError(
      "LLM_UNAVAILABLE",
      `No model provider produced a valid answer for "${task}". ${errors.join(" | ") || "No provider is configured."}`,
      { status: 503 },
    );
  }
  ...
  return {
    generateJSON,
    stats,
    providers: slots.map((s) => s.provider.name),
  ...
  for (const name of cfg.order) {
    if (name === "mock")
      slots.push({
        ...
        limiter: new RateLimiter({ rpm: 10_000, tpm: 1e9 }),
      });
    else if (name === "gemini" && cfg.gemini.apiKey)
      slots.push({
        provider: geminiProvider(cfg.gemini),
        limiter: new RateLimiter(cfg.gemini),
      });
    else if ((name === "groq" || name === "openrouter") && cfg[name].apiKey)
      slots.push({
        provider: openAICompatibleProvider({ kind: name, ...cfg[name] }),
        limiter: new RateLimiter(cfg[name]),
      });
  }
